...
from lib import log_command, re_rsplit, send_long_message
from handlers import admin, student, superadmin, start

from database import (
    init_db, add_student, update_student_login_password, delete_student,
...
# если у тебя есть функции регистрации админских/суперадминских хендлеров — раскомментируй:
# admin.register_admin_handlers(dp)
# superadmin.register_superadmin_handlers(dp)

# --- helpers ---

def _superadmin_alert_text(text: str) -> str:
    return f"⚠️ {text}"

async def _notify_superadmin(text: str):
    """Безопасно уведомить суперадмина по chat_id, если он задан и бот видел чат."""
    if not SUPERADMIN_ID:
        logger.warning("SUPERADMIN_ID не задан; пропускаю отправку алерта: %s", text)
        return
    try:
        await bot.send_message(SUPERADMIN_ID, _superadmin_alert_text(text))
    except Exception as e:
        logger.exception("Не удалось отправить сообщение суперадмину: %s", e)
...
async def list_all_students(message: types.Message):
    ...
    if students:
        result = '\n'.join([
            f"{s[0]} - Логин: {s[1]}, Пароль: {s[2]}, Программа: {s[3]}, Телеграм: {s[4]}"
            for s in students
        ])
        await send_long_message(message, f"Список всех студентов:\n{result}")
    else:
...
async def add_user_start(message: types.Message):
    ...
    await message.answer(
        "Введите данные для добавления пользователей (каждый пользователь на новой строке):\n"
...
async def process_add_user(message: types.Message, state: FSMContext):
    ...
    for user_data in users_data:
        ...
        if duplicate:
            await _notify_superadmin(f"Найдены похожие ФИО: {full_name} и {duplicate}")
...
async def update_user_start(message: types.Message):
    ...
    await message.answer(
        "Введите данные для обновления пользователей (каждый пользователь на новой строке):\n"
...
async def process_users_file(message: types.Message, state: FSMContext):
    ...
    with open('users.csv', newline='', encoding='utf-8') as csvfile:
        ...
        for line in content:
            row = re_rsplit(r'[,\s\t]+', line.strip(), maxsplit=3)
            if len(row) < 4:
...
async def process_list_program(message: types.Message, state: FSMContext):
    ...
    if students:
        result = '\n'.join([
            f"{s[0]} - Логин: {s[1]}, Пароль: {s[2]}, Телеграм: {s[3]}"
            for s in students
        ])
        await send_long_message(message, f"Студенты программы {program}:\n{result}")
    else:
...
if __name__ == "__main__":
    app = _create_app()
    port = int(os.getenv("PORT", "10000"))  # Render проставит PORT
